...
export async function createShortcutWrapper(version, serverIp = null) {
  if (!(await fs.exists(join(constants.MOONTWEAKS_DIR, 'wrappers'))))
    await fs.mkdir(join(constants.MOONTWEAKS_DIR, 'wrappers'));
  ...
  const metadata = await fetchMetadata(true, version);
